...
size_t base_user_agents_count = 6;

void init_user_agents() {
    // Initialize random seed
    srand(time(NULL));

    // Format the first two strings as Python does (once at startup)
    char ua_formatted1[256];
    sprintf(ua_formatted1, "Mozilla/%.1f (Windows; U; Windows NT %.1f; en-US; rv:%.1f.%.1f) Gecko/%d0%d Firefox/%.1f.%.1f",
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            rand() % 10, rand() % 10, // %d0%d
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0  // random.uniform(5.0, 10.0)
            );
    base_user_agents[0] = strdup(ua_formatted1);

    char ua_formatted2[256];
     sprintf(ua_formatted2, "Mozilla/%.1f (Windows; U; Windows NT %.1f; en-US; rv:%.1f.%.1f) Gecko/%d0%d Chrome/%.1f.%.1f",
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
            (double)(rand() % 10 + 5) / 1.0, // random.uniform(5.0, 10.0)
    ...
    base_user_agents[1] = strdup(ua_formatted2);

    // The remaining strings contain literal '%' placeholders as in the original Python
    base_user_agents[2] = strdup("Mozilla/%.1f (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/%.1f.%.1f (KHTML, like Gecko) Version/%d.0.%d Safari/%.1f.%.1f");
    base_user_agents[3] = strdup("Mozilla/%.1f (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/%.1f.%.1f (KHTML, like Gecko) Version/%d.0.%d Chrome/%.1f.%.1f");
    base_user_agents[4] = strdup("Mozilla/%.1f (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/%.1f.%.1f (KHTML, like Gecko) Version/%d.0.%d Firefox/%.1f.%.1f");
    base_user_agents[5] = strdup("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/37.0.2062.94 Chrome/37.0.2062.94 Safari/537.36");
}

void free_user_agents() {
    for (size_t i = 0; i < base_user_agents_count; ++i) {
